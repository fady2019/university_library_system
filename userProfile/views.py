...
def displayBooks(request, id):
    ...
    if searchKey.strip() == '':
        ...
        if searchOption == 'name':
            books = Book.objects.filter(name=searchKey)
        elif searchOption == 'ispn':
            books = Book.objects.filter(ISPN=searchKey)
        elif searchOption == 'author':
            books = Book.objects.filter(author=searchKey)
        elif searchOption == 'year':
            books = Book.objects.filter(
                publicationDate_from_year_gte=searchKey)
    ...
    if searchKey.strip() == '':
        ...
        if searchOption == 'name':
            books = BorrowBook.objects.filter(
                user=request.user,
                book__name=searchKey
            )
        elif searchOption == 'ispn':
            books = BorrowBook.objects.filter(
                user=request.user,
                book__ISPN=searchKey
            )
        elif searchOption == 'author':
            books = BorrowBook.objects.filter(
                user=request.user,
                book__author=searchKey
            )
        elif searchOption == 'year':
            books = BorrowBook.objects.filter(
                user=request.user,
                book__publicationDate_from_year_gte=searchKey
            )
